...
read (*,*) nguess
! Number of Kactual values that are given by the user from the results of 
! the previous fitting proceedure.
write (*,*) "The number of Kactual values that are given as a first guess is ",nguess,".\n\n"
do nguess=1,nguess
  read (*,*) i, j, Kactual(i,j)
enddo
...
subroutine PrintIterKactuals (KactualCollect,startTime,NsimMax,Nmax,PF,nsamp_gt_0,nc1,niter)
  ...
  character(1+(14+1)*50) :: StringKs, StringGs
  
  integer :: NsimMax(2), Nmax, niter
  integer :: i, j, k ! Counters
  integer :: PF, FN_K, FN_G
  
  real*8 :: KactualCollect(0:Nmax,0:Nmax,0:22+1), DeltaG, nc1
...
! moved to.
          ...
          if (nsamp_gt_0(i,j)) then
            do iset=1,nsets
              if (ci_gt_0(i,j,iset)) then
                ...
                !
                ! Since this is the second round of adjustments, we now use the 
                ! n_i,j values to create Kfac
                Kfac=Kfac+log(aveN(i,j,iset)/aveNout(i,j,iset))*(alpha*&
                NrelW(i,j,iset)/weightSum(i,j))
              endif
            enddo
  ...
  printCount=printCount+1
  KactualCollect(0:Nmax,0:Nmax,printCount)=Kactual(0:Nmax,0:Nmax)
  call PrintIterKactuals (KactualCollect,startTime,NsimMax,Nmax,PF,nsamp_gt_0,nc1,niter)
  return
end
